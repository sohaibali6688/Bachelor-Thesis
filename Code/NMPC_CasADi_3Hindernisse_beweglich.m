...
% Langsame Fahrzeuge auf der Referenz. Alle Werte liegen deutlich unter der
% Ego-Zielgeschwindigkeit auf Geraden, damit ein Ueberholmanoever entsteht.
obs_speed = [3.0; 3.0; 3.0];           % konstante Laengsgeschwindigkeit [m/s]
obs_speed = obs_speed(1:n_obs);
...
fprintf('\n===== Zeitmessung: 3 bewegliche Hindernisse =====\n');
...
disp(runtime_table);
writetable(runtime_table, 'runtime_3_bewegliche_Hindernisse.csv');
save('runtime_3_bewegliche_Hindernisse.mat', 'runtime_table', 'solver_time', 'step_time');
...
title(sprintf('NMPC mit 3 langsamen Fahrzeugen + Ueberholen + optimierter MPC-Vorschau | %d Runden | ohne feste V | v_{max}=%.1f m/s | N=%d | Ts=%.2f s', ...
    num_laps, v_max, N, Ts));
...
video_filename     = 'NMPC_multi_lap_3_langsame_Fahrzeuge_ueberholen4.mp4';
...
if animate_simulation
    ...
    xlabel('X [m]');
    ylabel('Y [m]');
    title('NMPC mit 3 langsamen Fahrzeugen auf der Referenz | Ueberholen mit MPC-Vorschau');
