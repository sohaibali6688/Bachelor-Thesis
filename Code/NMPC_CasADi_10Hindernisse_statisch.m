...
% ----------------------------
% 2b) Zehn statische Hindernisse auf MEINER Referenz
% ----------------------------
...
%
% Hinweis: 10 Hindernisse sind bewusst ein Stresstest. Falls IPOPT wegen zu
% vieler enger Situationen keine Loesung findet, zuerst obs_safe_margin oder
% obs_rad kleiner waehlen oder die Fraktionen weiter auseinander legen.
n_obstacles_requested = 10;
obs_frac = linspace(0.10, 0.90, n_obstacles_requested);  % gleichmaessig auf einer Runde verteilt
...
avoid_ref_side = repmat([1; -1], ceil(n_obs/2), 1);  % Hindernisse abwechselnd links/rechts umfahren
...
% Aktivierung pro Runde: physisch sind es 10 Hindernisse, aber der
...
fprintf('\n===== Zeitmessung: 10 statische Hindernisse =====\n');
...
disp(runtime_table);
writetable(runtime_table, 'runtime_10_statische_Hindernisse.csv');
save('runtime_10_statische_Hindernisse.mat', 'runtime_table', 'solver_time', 'step_time');
...
title(sprintf('NMPC mit 10 statischen Hindernissen | %d Runden | Gerade bis %.1f m/s | N=%d | Ts=%.2f s', ...
    num_laps, v_max, N, Ts));
...
video_filename     = 'NMPC_10_statische_Hindernisse.mp4';
...
if animate_simulation
    ...
    xlabel('X [m]');
    ylabel('Y [m]');
    title('NMPC mit 10 statischen Hindernissen | Fahrzeug folgt der optimierten MPC-Vorschau');
