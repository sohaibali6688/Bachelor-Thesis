...
% ----------------------------
% 2b) Zehn bewegliche langsame Fahrzeuge auf der Referenz
% ----------------------------
...
%
% Hinweis: 10 bewegliche Hindernisse sind bewusst ein Stresstest. Das NLP wird
% deutlich groesser, weil fuer jedes Hindernis und jeden Horizontschritt
% Sicherheitsbedingungen und Strafkosten berechnet werden.
n_obstacles_requested = 10;
obs_frac = linspace(0.10, 0.90, n_obstacles_requested);  % Startpositionen auf einer Runde
obs_idx_lap = unique(round(obs_frac * numel(X_lap)), 'stable');
...
% Langsame Fahrzeuge auf der Referenz. Unterschiedliche Geschwindigkeiten
% machen das Verhalten sichtbarer als 10 identische Hindernisse.
obs_speed = linspace(2.2, 5.5, n_obs).';   % konstante Laengsgeschwindigkeit [m/s]
% Alternative fuer gleiche Geschwindigkeit bei allen Hindernissen:
% obs_speed = 3.0 * ones(n_obs, 1);
...
avoid_ref_side = repmat([1; -1], ceil(n_obs/2), 1);  % Ueberholseite je Fahrzeug: +1 links, -1 rechts
...
% Zeitmessung
% ----------------------------
total_timer = tic;                 % gesamte NMPC-Simulationszeit ohne Plots
...
while true
    ...
    % Langsame Fahrzeuge ueber den ganzen MPC-Horizont vorhersagen.
    t_now = (t - 1) * Ts;
    [obs_pred_pos, obs_pred_s] = predictPathVehiclePositions(t_now, N, Ts, X_lap, Y_lap, s_lap, lap_length, obs_s0, obs_speed);
    ...
    x_current    = x_next;
    u_last       = u_opt;
    obs_next_tmp = predictPathVehiclePositions(t * Ts, 0, Ts, X_lap, Y_lap, s_lap, lap_length, obs_s0, obs_speed);
...
fprintf('\n===== Zeitmessung: 10 bewegliche Hindernisse =====\n');
...
disp(runtime_table);
writetable(runtime_table, 'runtime_10_bewegliche_Hindernisse.csv');
save('runtime_10_bewegliche_Hindernisse.mat', 'runtime_table', 'solver_time', 'step_time');
...
plotPathVehicleTraces(X_lap, Y_lap, s_lap, lap_length, obs_s0, obs_speed, 0, (size(X_log,2)-1)*Ts);
plotObstacleSet(Obsnow_log(:,:,end), obs_rad, obs_safe);
grid on; axis equal;
xlabel('X [m]');
ylabel('Y [m]');
legend('Location','best');
title(sprintf('NMPC mit 10 beweglichen langsamen Fahrzeugen + Ueberholen | %d Runden | ohne feste V | v_{max}=%.1f m/s | N=%d | Ts=%.2f s', ...
    num_laps, v_max, N, Ts));
...
video_filename     = 'NMPC_10_bewegliche_Hindernisse_ueberholen.mp4';
...
if animate_simulation
    ...
    xlabel('X [m]');
    ylabel('Y [m]');
    title('NMPC mit 10 beweglichen Hindernissen auf der Referenz | Ueberholen mit MPC-Vorschau');
