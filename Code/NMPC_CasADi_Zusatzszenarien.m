...
% langsamen vorausfahrenden Fahrzeugen und automatisch berechnetem
% Geschwindigkeitsprofil ohne feste Gerade-/Kurven-Sollgeschwindigkeit.
%
% Ziel:
...
% - langsame Fahrzeuge werden weiter im Horizont vorhergesagt und ueberholt
%
% Idee:
...
% 6) Der MPC bekommt entlang des Horizonts eine lokale Referenz
...
% ----------------------------
% Zusatzszenario fuer bewegliche Hindernisse
% 1 = Hindernisse beschleunigen, wenn das Ego-Fahrzeug zu nah kommt
% 2 = Hindernisse bremsen, wenn das Ego-Fahrzeug zu nah kommt
% 3 = normales Ueberholen, aber alle 3 Hindernisse haben unterschiedliche konstante Geschwindigkeiten
%
% Fuer einzelne Dateien unten einfach scenario_id = 1, 2 oder 3 setzen.
scenario_id = 1;
...
Q_obs = 7500;   % staerker, weil die Hindernisse wie langsamere Fahrzeuge direkt auf der Referenz fahren
...
corridor_radius = 8.0;   % [m], genug Platz fuer Ueberholspur neben der Referenz
...
% ----------------------------
% 2b) Drei langsame Fahrzeuge auf der Referenz
% ----------------------------
% Die Hindernisse sind jetzt keine quer laufenden Objekte mehr, sondern
% langsame vorausfahrende Fahrzeuge. Sie fahren auf derselben roten Referenz
% mit eigener langsamer Geschwindigkeit. Der ego-MPC nutzt die vorhergesagten
% Positionen im Horizont, weicht seitlich aus, ueberholt und kommt danach
% wieder auf die Originalreferenz zurueck.
obs_idx_lap = unique(round([0.22, 0.53, 0.79] * numel(X_lap)));
obs_idx_lap = max(1, min(numel(X_lap), obs_idx_lap(:)));
n_obs = numel(obs_idx_lap);

% Startpositionen der langsamen Fahrzeuge als Bogenlaenge auf einer Runde
obs_s0 = s_lap(obs_idx_lap(:));
obs_x0 = X_lap(obs_idx_lap);
obs_y0 = Y_lap(obs_idx_lap);
obs_pos0 = [obs_x0(:), obs_y0(:)];

% Die Hindernisse sind fahrzeugaehnlich. Der Kreis ist konservativ gewaehlt.
obs_rad = 1.75 * ones(n_obs, 1);       % Fahrzeug-/Hindernisradius [m]
obs_safe_margin = 1.55;                % Sicherheitsabstand [m]
obs_influence_margin = 7.00;           % Einflussbereich fuer fruehes Ueberholen [m]
obs_safe = obs_rad + obs_safe_margin;
obs_infl = obs_safe + obs_influence_margin;

% Langsame Fahrzeuge auf der Referenz.
% In den Zusatzszenarien wird entweder die Geschwindigkeit der Hindernisse
% reaktiv veraendert oder es werden drei verschiedene konstante
% Hindernisgeschwindigkeiten verwendet.
switch scenario_id
    case 1
        scenario_name = 'Szenario 1: Hindernisse beschleunigen bei zu geringem Abstand';
        scenario_tag  = 'beschleunigen';
        obs_speed_nom = 3.0 * ones(n_obs, 1);       % Nominalgeschwindigkeit [m/s]
        obs_dynamic_speed = true;
        obs_reaction_mode = 'accelerate';
    case 2
        scenario_name = 'Szenario 2: Hindernisse bremsen bei zu geringem Abstand';
        scenario_tag  = 'abbremsen';
        obs_speed_nom = 3.0 * ones(n_obs, 1);       % Nominalgeschwindigkeit [m/s]
        obs_dynamic_speed = true;
        obs_reaction_mode = 'brake';
    case 3
        scenario_name = 'Szenario 3: drei Hindernisse mit verschiedenen Geschwindigkeiten';
        scenario_tag  = 'verschiedene_geschwindigkeiten';
        obs_speed_nom = [2.5; 3.5; 5.0];            % verschiedene konstante Geschwindigkeiten [m/s]
        obs_dynamic_speed = false;
        obs_reaction_mode = 'none';
    otherwise
        error('Unbekannte scenario_id. Erlaubt sind 1, 2 oder 3.');
end

obs_speed_nom = obs_speed_nom(1:n_obs);
obs_speed_state = obs_speed_nom;   % aktuelle Hindernisgeschwindigkeit [m/s]
obs_s_state = obs_s0(:);           % aktuelle Bogenlaenge der Hindernisse [m]

% Parameter fuer Szenario 1 und 2.
% "Zu nah" wird ueber euklidischen Abstand und Laengsabstand auf der Strecke
% erkannt. Damit reagieren nur Hindernisse, die vor oder fast neben dem Ego-
% Fahrzeug liegen, nicht aber bereits ueberholte Fahrzeuge.
obs_react_distance_margin  = 8.0;    % zusaetzlich zum Sicherheitsradius [m]
obs_react_longitudinal_m   = 35.0;   % Hindernis muss maximal so weit voraus sein [m]
obs_react_behind_tol_m     = 5.0;    % kleine Toleranz, wenn Ego schon fast neben dem Hindernis ist [m]
obs_accel_when_close       = 2.0;    % Beschleunigung fuer Szenario 1 [m/s^2]
obs_brake_when_close       = 3.0;    % Bremsverzoegerung fuer Szenario 2 [m/s^2]
obs_speed_min              = 0.8;    % Untergrenze Hindernisgeschwindigkeit [m/s]
obs_speed_max              = 12.0;   % Obergrenze Hindernisgeschwindigkeit [m/s]
obs_return_gain            = 0.6;    % Rueckkehr zur Nominalgeschwindigkeit [1/s]

% Lokale Ueberholreferenz:
% Die rote Originalreferenz bleibt unveraendert. Nur im MPC-Horizont wird die
% Sollspur seitlich neben das langsamere Fahrzeug gelegt. Nach dem Hindernis
% faellt der Offset wieder auf 0, sodass das Fahrzeug zur Referenz zurueckkehrt.
avoid_ref_half_width_m = 28.0;         % laengsseitiger Einflussbereich entlang der Referenz [m]
avoid_ref_offset_extra = 1.25;         % zusaetzlicher seitlicher Abstand ausserhalb des Sicherheitsradius [m]
avoid_ref_side = [1; -1; 1];           % Ueberholseite je Fahrzeug: +1 links, -1 rechts
avoid_ref_side = avoid_ref_side(1:n_obs);
avoid_v_factor = 0.85;                % nur leicht bremsen; Ziel ist Ueberholen statt Hinterherfahren
avoid_v_min = 5.5;                     % Mindest-v_ref im Ueberholbereich [m/s]
...
% Bewegliche Hindernisparameter fuer diesen MPC-Schritt:
% Spalten = k=1..N+1, Zeilen je Hindernis = [x; y; Sicherheitsradius; Einflussradius]
obs_param = MX.sym('obs_param', 4*n_obs, N+1);
...
for k = 1:N
    ...
    for j = 1:n_obs
        ...
        ox    = obs_param(r0 + 1,k);
        oy    = obs_param(r0 + 2,k);
        osafe = obs_param(r0 + 3,k);
        oinfl = obs_param(r0 + 4,k);
...
for j = 1:n_obs
    r0N = 4 * (j - 1);
    oxN    = obs_param(r0N + 1,N+1);
    oyN    = obs_param(r0N + 2,N+1);
    osafeN = obs_param(r0N + 3,N+1);
    oinflN = obs_param(r0N + 4,N+1);
...
Obspred_log = cell(1, alloc_steps);
Obsnow_log = zeros(n_obs, 2, alloc_steps+1);
Obsnow_log(:,:,1) = obs_pos0;
ObsSpeed_log = zeros(n_obs, alloc_steps+1);
ObsSpeed_log(:,1) = obs_speed_state;
...
% Zeitmessung
% ----------------------------
total_timer = tic;                 % gesamte Rechenzeit der MPC-Schleife ohne Plots
solver_time = zeros(alloc_steps,1);
step_time   = zeros(alloc_steps,1);
...
while true
    ...
    if t > size(U_log,2)
        ...
        Refprev_log{newCap} = [];
        Obspred_log{newCap} = [];
        Obsnow_log(:,:,newCap+1) = 0;
        ObsSpeed_log(:,newCap+1) = 0;

        solver_time(newCap,1) = 0;
        step_time(newCap,1)   = 0;
    end
    ...
    % Bewegliche Hindernisse ueber den ganzen MPC-Horizont vorhersagen.
    % In Szenario 1/2 wird zuerst die aktuelle Hindernisgeschwindigkeit
    % anhand des Abstands zum Ego-Fahrzeug aktualisiert. Anschliessend wird
    % der Horizont mit dieser aktuellen Geschwindigkeit vorhergesagt.
    [obs_now_x, obs_now_y] = interpolateClosedPathXY(obs_s_state, X_lap, Y_lap, s_lap, lap_length);
    obs_now_pos = [obs_now_x(:), obs_now_y(:)];
    ego_s_mod = mod(s_path(path_idx), lap_length);

    [obs_speed_state, obs_a_cmd, too_close_obs] = updateObstacleSpeedByEgoDistance( ...
        obs_speed_state, obs_speed_nom, obs_now_pos, obs_s_state, ego_s_mod, x_current(1:2), obs_safe, Ts, ...
        obs_dynamic_speed, obs_reaction_mode, obs_react_distance_margin, obs_react_longitudinal_m, ...
        obs_react_behind_tol_m, obs_accel_when_close, obs_brake_when_close, obs_speed_min, obs_speed_max, ...
        obs_return_gain, lap_length); %#ok<NASGU>

    [obs_pred_pos, obs_pred_s] = predictPathVehiclePositionsFromState(N, Ts, X_lap, Y_lap, s_lap, lap_length, obs_s_state, obs_speed_state);

    % Lokale Ueberholreferenz erzeugen: seitlich am langsameren Fahrzeug
    % vorbei und danach wieder zurueck auf die Originalreferenz.
    ref_s_horizon = s_path(idx);
    ref_mat = buildOvertakeReferenceOnPath(ref_mat_nominal, ref_s_horizon, obs_pred_s, obs_safe, ...
        avoid_ref_half_width_m, avoid_ref_offset_extra, avoid_ref_side, ...
        avoid_v_factor, avoid_v_min, lap_length);

    % Fuer Plot/Animation die wirklich im MPC verwendeten Werte speichern.
    vref_log(t) = ref_mat(5,1);
    Refprev_log{t} = ref_mat;
    Obspred_log{t} = obs_pred_pos;
    Obsnow_log(:,:,t) = obs_pred_pos(:,:,1);

    % Hindernisparameter zeitabhaengig fuer jeden Horizontschritt.
    obs_param_num = buildMovingObstacleParamMatrix(obs_pred_pos, obs_safe, obs_infl);
    ...
    solver_time(t) = toc(solver_timer);
    ...
    x_current    = x_next;
    u_last       = u_opt;
    % Hindernisse um einen Abtastschritt weiterbewegen.
    obs_s_state = mod(obs_s_state + obs_speed_state * Ts, lap_length);
    obs_next_tmp = predictPathVehiclePositionsFromState(0, Ts, X_lap, Y_lap, s_lap, lap_length, obs_s_state, obs_speed_state);
    Obsnow_log(:,:,t+1) = obs_next_tmp(:,:,1);
    ObsSpeed_log(:,t+1) = obs_speed_state;

    step_time(t) = toc(step_timer);
    ...
    if path_idx >= numel(X_path) - 6
        ...
        Obspred_log = Obspred_log(1:t);
        Obsnow_log = Obsnow_log(:,:,1:t+1);
        ObsSpeed_log = ObsSpeed_log(:,1:t+1);
        break;
    end
end
...
% ----------------------------
total_runtime = toc(total_timer);

solver_time = solver_time(1:t);
step_time   = step_time(1:t);

simulation_time = t * Ts;

total_solver_time = sum(solver_time);
mean_solver_time  = mean(solver_time);
max_solver_time   = max(solver_time);
min_solver_time   = min(solver_time);

mean_step_time = mean(step_time);
max_step_time  = max(step_time);

real_time_factor = total_runtime / simulation_time;

fprintf('\n===== Zeitmessung: %s =====\n', scenario_name);
fprintf('Simulierte Fahrzeit:          %.3f s\n', simulation_time);
fprintf('Gesamte Rechenzeit:           %.3f s\n', total_runtime);
fprintf('Gesamte Solverzeit:           %.3f s\n', total_solver_time);
fprintf('Mittlere Solverzeit/Schritt:  %.4f s\n', mean_solver_time);
fprintf('Maximale Solverzeit/Schritt:  %.4f s\n', max_solver_time);
fprintf('Minimale Solverzeit/Schritt:  %.4f s\n', min_solver_time);
fprintf('Mittlere Schrittzeit:         %.4f s\n', mean_step_time);
fprintf('Maximale Schrittzeit:         %.4f s\n', max_step_time);
fprintf('Echtzeitfaktor:               %.2f\n', real_time_factor);
fprintf('===============================================\n');
...
runtime_csv = ['runtime_zusatz_' scenario_tag '.csv'];
runtime_mat = ['runtime_zusatz_' scenario_tag '.mat'];

writetable(runtime_table, runtime_csv);
save(runtime_mat, 'runtime_table', 'solver_time', 'step_time');
...
plotLoggedPathVehicleTraces(Obsnow_log);
plotObstacleSet(Obsnow_log(:,:,end), obs_rad, obs_safe);
grid on; axis equal;
xlabel('X [m]');
ylabel('Y [m]');
legend('Location','best');
title(sprintf('%s | %d Runden | v_{max}=%.1f m/s | N=%d | Ts=%.2f s', ...
    scenario_name, num_laps, v_max, N, Ts));
...
figure('Position',[160,160,820,360]);
plot(0:size(ObsSpeed_log,2)-1, ObsSpeed_log', 'LineWidth', 1.7);
grid on;
xlabel('Schritt');
ylabel('v_{obs} [m/s]');
title(['Hindernisgeschwindigkeiten - ', scenario_name]);
legend(arrayfun(@(j) sprintf('Hindernis %d', j), 1:n_obs, 'UniformOutput', false), 'Location', 'best');

% ----------------------------
% 7) Animation
% ----------------------------
animate_simulation = true;   % true -> Animation nach der Simulation anzeigen
animation_stride   = 5;      % nur jeden n-ten Simulationsschritt zeichnen
animation_pause    = 0.05;   % Pause zwischen zwei Frames [s]
show_preview       = true;   % optimierte MPC-Vorschau anzeigen
save_video         = false;  % optional als MP4 speichern
video_filename     = sprintf('NMPC_Zusatzszenario_%d_%s.mp4', scenario_id, scenario_tag);
...
if animate_simulation
    ...
    [h_obs_body, h_obs_safe] = initMovingObstacleGraphics(Obsnow_log(:,:,1), obs_rad, obs_safe);
    ...
    xlabel('X [m]');
    ylabel('Y [m]');
    title(['NMPC Zusatzszenario | ', scenario_name]);
    lgd = legend('Referenz (geschlossen)', 'langsame Fahrzeuge', 'Sicherheitsradius', ...
...
h_info = annotation(fig_anim, 'textbox', [0.68 0.38 0.28 0.25], ...
    ...
    for k = 1:animation_stride:size(X_log,2)
        ...
        obs_now_anim = Obsnow_log(:,:,min(k, size(Obsnow_log,3)));
        updateObstacleGraphics(h_obs_body, h_obs_safe, obs_now_anim, obs_rad, obs_safe);
        ...
        obs_now_info = Obsnow_log(:,:,min(k, size(Obsnow_log,3)));
        dist_obs_info = sqrt((obs_now_info(:,1) - X_log(1,k)).^2 + (obs_now_info(:,2) - X_log(2,k)).^2) - obs_safe;
        min_dist_obs_info = min(dist_obs_info);
...
function [X_ref, Y_ref, phi_ref, delta_ref, s_ref, kappa, v_ref] = ...
    ...
    % ----------------------------
    % Diese Version ist absichtlich nah an der Original-Logik:
    ...
    % Damit bleibt das Kurvenniveau nah am Original, ohne feste Kurven-Sollgeschwindigkeit
    % einzutragen.
...
function [tangent, normal_left] = computePathFrameAtIndices(X_path, Y_path, indices)
    n_idx = numel(indices);
    n_path = numel(X_path);
    tangent = zeros(n_idx, 2);
    normal_left = zeros(n_idx, 2);

    for ii = 1:n_idx
        idx = max(1, min(n_path, indices(ii)));
        idx_prev = idx - 1;
        idx_next = idx + 1;
        if idx_prev < 1
            idx_prev = n_path;
        ...
        tx = X_path(idx_next) - X_path(idx_prev);
        ty = Y_path(idx_next) - Y_path(idx_prev);
        tn = hypot(tx, ty);
        if tn < 1e-9
            tx = 1;
            ty = 0;
        ...
        tangent(ii,:) = [tx, ty];
        normal_left(ii,:) = [-ty, tx];
    end

end

function [obs_pred, obs_s_pred] = predictPathVehiclePositions(t_now, N, Ts, X_lap, Y_lap, s_lap, lap_length, obs_s0, obs_speed)
    % Langsame Fahrzeuge bewegen sich direkt auf der geschlossenen Referenz.
    % obs_s_pred(j,k) ist die Bogenlaenge des j-ten Fahrzeugs zum k-ten
    % Horizontzeitpunkt, modulo lap_length.
    n_obs_local = numel(obs_s0);
    obs_pred = zeros(n_obs_local, 2, N+1);
    obs_s_pred = zeros(n_obs_local, N+1);
    ...
    for kk = 1:N+1
        tau = t_now + (kk - 1) * Ts;
        s_query = mod(obs_s0 + obs_speed * tau, lap_length);
        obs_s_pred(:,kk) = s_query;
        [xq, yq] = interpolateClosedPathXY(s_query, X_lap, Y_lap, s_lap, lap_length);
        obs_pred(:,:,kk) = [xq(:), yq(:)];
    end
end

function [obs_pred, obs_s_pred] = predictPathVehiclePositionsFromState(N, Ts, X_lap, Y_lap, s_lap, lap_length, obs_s_state, obs_speed_state)
    % Vorhersage ab dem aktuellen Hinderniszustand.
    % Diese Funktion wird fuer Szenarien mit zeitvariabler Hindernisgeschwindigkeit
    % verwendet. Innerhalb eines MPC-Horizonts wird die aktuelle Geschwindigkeit
    % konstant gehalten; im naechsten MPC-Schritt wird sie erneut aktualisiert.
    n_obs_local = numel(obs_s_state);
    obs_pred = zeros(n_obs_local, 2, N+1);
    obs_s_pred = zeros(n_obs_local, N+1);

    obs_s_state = obs_s_state(:);
    obs_speed_state = obs_speed_state(:);

    for kk = 1:N+1
        tau = (kk - 1) * Ts;
        s_query = mod(obs_s_state + obs_speed_state * tau, lap_length);
        obs_s_pred(:,kk) = s_query;
        [xq, yq] = interpolateClosedPathXY(s_query, X_lap, Y_lap, s_lap, lap_length);
        obs_pred(:,:,kk) = [xq(:), yq(:)];
    end
...
function [obs_speed_state, obs_a_cmd, too_close] = updateObstacleSpeedByEgoDistance( ...
    obs_speed_state, obs_speed_nom, obs_now_pos, obs_s_state, ego_s_mod, ego_xy, obs_safe, Ts, ...
    obs_dynamic_speed, obs_reaction_mode, react_margin, react_longitudinal_m, react_behind_tol_m, ...
    accel_when_close, brake_when_close, speed_min, speed_max, return_gain, lap_length)

    obs_speed_state = obs_speed_state(:);
    obs_speed_nom   = obs_speed_nom(:);
    obs_s_state     = obs_s_state(:);
    obs_safe        = obs_safe(:);
    ego_xy          = ego_xy(:).';

    n_obs_local = numel(obs_speed_state);
    obs_a_cmd = zeros(n_obs_local, 1);
    too_close = false(n_obs_local, 1);

    if ~obs_dynamic_speed
        return;
    end

    dist_xy = sqrt(sum((obs_now_pos - ego_xy).^2, 2));
    signed_s_ego_to_obs = wrapSignedDistanceOnClosedPath(obs_s_state - ego_s_mod, lap_length);
    react_dist = obs_safe + react_margin;

    too_close = (dist_xy <= react_dist) & ...
                (signed_s_ego_to_obs >= -react_behind_tol_m) & ...
                (signed_s_ego_to_obs <=  react_longitudinal_m);

    for j = 1:n_obs_local
        if too_close(j)
            switch lower(obs_reaction_mode)
                case 'accelerate'
                    obs_a_cmd(j) = accel_when_close;
                case 'brake'
                    obs_a_cmd(j) = -brake_when_close;
                otherwise
                    obs_a_cmd(j) = 0;
            end
        else
            % Wenn kein Ego-Fahrzeug mehr zu nah ist, kehrt das Hindernis
            % langsam zur Nominalgeschwindigkeit zurueck.
            obs_a_cmd(j) = return_gain * (obs_speed_nom(j) - obs_speed_state(j));
        end
    end

    obs_a_cmd = min(max(obs_a_cmd, -brake_when_close), accel_when_close);
    obs_speed_state = obs_speed_state + Ts * obs_a_cmd;
    obs_speed_state = min(max(obs_speed_state, speed_min), speed_max);
end

function ref_mat = buildOvertakeReferenceOnPath(ref_mat, ref_s_horizon, obs_s_pred, obs_safe, ...
    avoid_half_width_m, avoid_offset_extra, avoid_side, avoid_v_factor, avoid_v_min, lap_length)

    if isempty(obs_s_pred)
        return;
    end

    n_steps = size(ref_mat, 2);
    n_obs_local = size(obs_s_pred, 1);
    avoid_half_width_m = max(avoid_half_width_m, 1e-3);
    ref_s_horizon = ref_s_horizon(:).';
    ...
    for k = 1:n_steps
        ...
        ref_s_mod = mod(ref_s_horizon(k), lap_length);

        for j = 1:n_obs_local
            % signed_s > 0: Referenzpunkt liegt vor dem langsamen Fahrzeug.
            % signed_s < 0: Referenzpunkt liegt hinter dem langsamen Fahrzeug.
            signed_s = wrapSignedDistanceOnClosedPath(ref_s_mod - obs_s_pred(j,k), lap_length);
            dist_s = abs(signed_s);

            if dist_s <= avoid_half_width_m
                shape = 0.5 * (1 + cos(pi * signed_s / avoid_half_width_m));

                % Feste Ueberholseite pro langsamem Fahrzeug. Das vermeidet ein
                % Umschalten links/rechts direkt am Hindernis und ergibt eine
                % glatte MPC-Vorschau.
                side_sign = avoid_side(j);
                offset_mag = (obs_safe(j) + avoid_offset_extra) * shape;
                offset_xy = offset_xy + side_sign * offset_mag * normal_left;

                % Nur hinter bzw. direkt neben dem langsamen Fahrzeug leicht
                % reduzieren. Nach dem Ueberholen wird die Referenzgeschwindigkeit
                % wieder freigegeben.
                if signed_s <= 0
...
function d = wrapSignedDistanceOnClosedPath(d, lap_length)
    d = mod(d + 0.5 * lap_length, lap_length) - 0.5 * lap_length;
end

function [xq, yq] = interpolateClosedPathXY(s_query, X_lap, Y_lap, s_lap, lap_length)
    s_query = mod(s_query(:), lap_length);

    s_nodes = [s_lap(:); lap_length];
    x_nodes = [X_lap(:); X_lap(1)];
    y_nodes = [Y_lap(:); Y_lap(1)];

    % Doppelte s-Werte vermeiden, falls die Referenz am Ende bereits exakt
    % den Startpunkt enthaelt.
    [s_unique, ia] = unique(s_nodes, 'stable');
    x_unique = x_nodes(ia);
    y_unique = y_nodes(ia);

    xq = interp1(s_unique, x_unique, s_query, 'linear', 'extrap');
    yq = interp1(s_unique, y_unique, s_query, 'linear', 'extrap');
end

function plotPathVehicleTraces(X_lap, Y_lap, s_lap, lap_length, obs_s0, obs_speed, t0, t1)
    if t1 <= t0
        t1 = t0 + 1;
    end

    t_vec = linspace(t0, t1, 160);
    n_obs_local = numel(obs_s0);

    for j = 1:n_obs_local
        s_trace = mod(obs_s0(j) + obs_speed(j) * t_vec(:), lap_length);
        [x_trace, y_trace] = interpolateClosedPathXY(s_trace, X_lap, Y_lap, s_lap, lap_length);

        if j == 1
            plot(x_trace, y_trace, 'k-.', 'LineWidth', 1.1, 'DisplayName', 'Bewegung langsame Fahrzeuge');
        else
            plot(x_trace, y_trace, 'k-.', 'LineWidth', 1.1, 'HandleVisibility', 'off');
        end
    end
end

function plotLoggedPathVehicleTraces(Obsnow_log)
    % Zeichnet die tatsaechlich simulierten Hindernisbahnen. Das ist wichtig
    % fuer Szenario 1/2, weil die Hindernisgeschwindigkeit nicht konstant ist.
    n_obs_local = size(Obsnow_log, 1);
    for j = 1:n_obs_local
        x_trace = squeeze(Obsnow_log(j,1,:));
        y_trace = squeeze(Obsnow_log(j,2,:));
        if j == 1
            plot(x_trace, y_trace, 'k-.', 'LineWidth', 1.1, 'DisplayName', 'Bewegung der Hindernisse');
        else
            plot(x_trace, y_trace, 'k-.', 'LineWidth', 1.1, 'HandleVisibility', 'off');
        end
    end
...
function obs_pred = predictMovingObstaclePositions(t_now, N, Ts, obs_cross_pos, obs_vel, obs_cross_time)
    n_obs_local = size(obs_cross_pos, 1);
    obs_pred = zeros(n_obs_local, 2, N+1);
    obs_cross_time = obs_cross_time(:);

    for kk = 1:N+1
        tau = t_now + (kk - 1) * Ts;
        dt_obs = tau - obs_cross_time;
        obs_pred(:,:,kk) = obs_cross_pos + bsxfun(@times, obs_vel, dt_obs);
    end
end

function obs_param_num = buildMovingObstacleParamMatrix(obs_pred_pos, obs_safe, obs_infl)
    n_obs_local = size(obs_pred_pos, 1);
    n_steps = size(obs_pred_pos, 3);
    obs_param_num = zeros(4*n_obs_local, n_steps);

    for kk = 1:n_steps
        for jj = 1:n_obs_local
            r0 = 4 * (jj - 1);
            obs_param_num(r0+1:r0+4, kk) = [obs_pred_pos(jj,1,kk); ...
                                            obs_pred_pos(jj,2,kk); ...
                                            obs_safe(jj); ...
                                            obs_infl(jj)];
        end
    end
...
function ref_mat = buildAvoidanceReferenceDynamic(ref_mat, obs_pred_pos, obs_safe, ...
    avoid_influence_m, avoid_offset_extra, avoid_side, avoid_v_factor, avoid_v_min)

    if isempty(obs_pred_pos)
        return;
    end

    n_steps = size(ref_mat, 2);
    n_obs_local = size(obs_pred_pos, 1);
    avoid_influence_m = max(avoid_influence_m, 1e-3);

    for k = 1:n_steps
        ref_xy = ref_mat(1:2,k);
        phi_ref_k = ref_mat(3,k);
        normal_left = [-sin(phi_ref_k); cos(phi_ref_k)];
        offset_xy = [0; 0];
        v_scale = 1.0;
        ...
        for j = 1:n_obs_local
            obs_xy = [obs_pred_pos(j,1,k); obs_pred_pos(j,2,k)];
            diff_xy = ref_xy - obs_xy;
            dist_xy = sqrt(diff_xy(1)^2 + diff_xy(2)^2);

            if dist_xy <= avoid_influence_m
                shape = 0.5 * (1 + cos(pi * dist_xy / avoid_influence_m));

                % Seitlich vom Hindernis weg. Wenn das Hindernis exakt auf der
                % Referenz liegt, wird die vorgegebene Fallback-Seite verwendet.
                side_sign = sign(diff_xy' * normal_left);
                if abs(side_sign) < 0.5
                    side_sign = avoid_side(j);
                end
...
function plotMovingObstacleTraces(obs_cross_pos, obs_vel, obs_cross_time, t0, t1)
    if t1 <= t0
        t1 = t0 + 1;
    end
    t_vec = linspace(t0, t1, 120);
    n_obs_local = size(obs_cross_pos, 1);

    for j = 1:n_obs_local
        x_trace = obs_cross_pos(j,1) + obs_vel(j,1) * (t_vec - obs_cross_time(j));
        y_trace = obs_cross_pos(j,2) + obs_vel(j,2) * (t_vec - obs_cross_time(j));
...
function [h_body, h_safe] = initMovingObstacleGraphics(obs_pos, obs_rad, obs_safe)
    th = linspace(0, 2*pi, 120);
    n_obs_local = size(obs_pos, 1);
    h_body = gobjects(n_obs_local, 1);
    h_safe = gobjects(n_obs_local, 1);
    ...
    for i = 1:n_obs_local
        ...
        if i == 1
            h_body(i) = fill(x1, y1, [0.85 0.2 0.2], 'FaceAlpha', 0.35, ...
                'EdgeColor', 'none', 'DisplayName', 'langsame Fahrzeuge');
            h_safe(i) = plot(x2, y2, 'k:', 'LineWidth', 1.3, 'DisplayName', 'Sicherheitsradius');
        else
            h_body(i) = fill(x1, y1, [0.85 0.2 0.2], 'FaceAlpha', 0.35, ...
                'EdgeColor', 'none', 'HandleVisibility', 'off');
            h_safe(i) = plot(x2, y2, 'k:', 'LineWidth', 1.3, 'HandleVisibility', 'off');
        end
    end
end

function updateObstacleGraphics(h_body, h_safe, obs_pos, obs_rad, obs_safe)
    th = linspace(0, 2*pi, 120);
    ...
    for i = 1:size(obs_pos, 1)
        ...
        set(h_body(i), 'XData', x1, 'YData', y1);
        set(h_safe(i), 'XData', x2, 'YData', y2);
    end
end
