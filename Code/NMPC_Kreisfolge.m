%% LPV_MPC_bicycle_circle.m
% LPV-MPC für kinematisches Einspurmodell
% Idee: pro Zeitschritt um aktuellen Zustand/aktuelles u linearisieren
%       => lineares affines Modell x+ = A x + B u + c
%       => QP (quadprog) statt fmincon
...
for k = 1:N
    ...
    x_next = f_bicycle_euler(x(:,k), u(:,k), Ts, lr, L);
    ...
    e_phi = x(3,k)-phi_ref(t+k-1);          % optional
    e_vel = x(4,k) - v_ref;
...
legend('Referenz (voller Kreis)','NMPC','Location','best');
title(sprintf('NMPC Einspurmodell: Kreisfolge, N=%d, Ts=%.2f', N, Ts));
